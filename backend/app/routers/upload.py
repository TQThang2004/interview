...
async def upload_image(file: UploadFile = File(...), current_user: dict = Depends(get_current_user)):
    ...
    if file.content_type not in allowed_types:
        raise HTTPException(status_code=400, detail="Chá»‰ há»— trá»£ file áº£nh (JPEG, PNG, WEBP, GIF).")
    if file.size is not None and file.size > MAX_IMAGE_UPLOAD_BYTES:
        raise HTTPException(status_code=413, detail="áº¢nh vÆ°á»£t quÃ¡ giá»›i háº¡n 5MB.")

    try:
        # Upload tháº³ng file-like object lÃªn Cloudinary
        result = cloudinary.uploader.upload(file.file)
        url = result.get("secure_url")
    except Exception as e:
        logger.exception("Failed to upload image to Cloudinary")
        raise HTTPException(status_code=500, detail="KhÃ´ng thá»ƒ lÆ°u file áº£nh lÃªn Cloudinary.")
